(() => {
  const SOURCES = [
    { key: "A",        label: "A",        url: "https://raw.githubusercontent.com/TORCHIN-Maxence-24020376/EDT/main/edt_data/2GA1-2.ics" },
    { key: "B",        label: "B",        url: "https://raw.githubusercontent.com/TORCHIN-Maxence-24020376/EDT/main/edt_data/2GB-1.ics" },
    { key: "Antoine",  label: "Antoine",  url: "https://raw.githubusercontent.com/TORCHIN-Maxence-24020376/EDT/main/edt_data/1G2B.ics" },
    { key: "Maxence",  label: "Maxence",  url: "https://raw.githubusercontent.com/TORCHIN-Maxence-24020376/EDT/main/edt_data/2GA1-1.ics" },
  ];

  // --- Persist sélection EDT ---
  const LS_KEY_EDT = "edt_selected_idx_v1";
  let currentIdx = Number(localStorage.getItem(LS_KEY_EDT) ?? 0); // 0..3
  if (!Number.isInteger(currentIdx) || currentIdx < 0 || currentIdx >= SOURCES.length) currentIdx = 0;

  function setCurrentIdx(idx){
    if (idx === currentIdx) return;
    currentIdx = idx;
    localStorage.setItem(LS_KEY_EDT, String(idx));
    loadAndRender();
  }
  ...
  async function loadICS(url){
    ...
    if (!resp.ok) throw new Error("Erreur de chargement de l’ICS: "+url);
    const events = parseICS(await resp.text());
    ICS_CACHE.set(url, events);
    return events;
  }

  // Header avec icônes + boutons EDT
  function makeHeader(container, weekStart, weekEnd) {
    ...
    Object.assign(left.style,{display:"flex",gap:"0.5rem",alignItems:"center"});

    const mkIconBtn = (iconPath, label, onClick) => {
    ...
    const prev  = mkIconBtn("img/prev.svg",  "Semaine précédente", ()=>{ weekOffset--; loadAndRender(); });
    const today = mkIconBtn("img/today.svg", "Aujourd’hui",         ()=>{ weekOffset=0;  loadAndRender(); });
    const next  = mkIconBtn("img/next.svg",  "Semaine suivante",    ()=>{ weekOffset++; loadAndRender(); });

    // Groupe de boutons EDT
    const edtGroup = document.createElement("div");
    Object.assign(edtGroup.style,{display:"inline-flex", gap:"0.25rem", marginLeft:"0.5rem"});

    const mkEdtBtn = (idx, label) => {
      ...
      Object.assign(b.style,{
        ...
        background: isActive ? "#5a5a5a" : "var(--glass-bg-dark)",
      ...
      b.addEventListener("mouseenter", ()=>{ if(!isActive) b.style.filter="brightness(1.1)"; });
    ...
    SOURCES.forEach((s, idx)=> edtGroup.appendChild(mkEdtBtn(idx, s.label)));

    left.append(prev, today, next, edtGroup);
  ...
  async function loadAndRender(){
    ...
    // charge l’EDT sélectionné
    const url = SOURCES[currentIdx].url;
    let events;
    try { events = await loadICS(url); }
  ...
  // Raccourcis clavier : ← → (semaine), T (cette semaine), 1..4 (changer d’EDT)
